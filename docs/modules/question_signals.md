...
`src/beacon/question_signals.py` extracts database-agnostic hints from the original question. It replaces the old idea that the first step should choose exact tables by hardcoded keywords.
...
flowchart TD
    ...
    B --> C["Extract terms, dates, numbers, entities"]
    B --> D["Match generic intents and metrics"]
    B --> E["Weak lexical match against semantic model"]
    C --> F["Question signals dict"]
    D --> F
    E --> F
```
